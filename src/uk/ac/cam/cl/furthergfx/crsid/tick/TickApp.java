...
public class TickApp extends TickCanvas {
  ...
  protected void render() {
    // Super render polls inputs, leaving it in the middle would
    // Cause the screenshot to fire at when the button is pressed not at target
    super.render();
  ...
  protected void onKeyPressed(int key) {
    switch (key) {
      ...
      case GLFW.GLFW_KEY_S:
        String[] options =
            new String[] {"Task 1", "Task 2", "Task 3", "Task 4", "Task 5", "Task 6"};
        Vector3f[] cameraPositions =
            new Vector3f[] {new Vector3f(0.4849999f, 0.32499993f, 11.81964f),
        ...
        int response = JOptionPane.showOptionDialog(null,
            ...
            "Select task", JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null, options,
            options[0]);
        if (response >= 0) {
          screenshotTarget = options[response].replace(" ", "") + ".png";
          if (response < 5) {
            camera.animateTo(cameraPositions[response].x, cameraPositions[response].y,
                cameraPositions[response].z);
          }
        }
        break;
    }
  ...
  private void updateResolutionUniform() {
    ...
    updateUniformVec2("resolution", width[0], height[0]);
  }
  ...
  private void takeScreenshot(String path) {
    ...
    System.out.println("Took screenshot: " + path);
  }
}
